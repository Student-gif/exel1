...
class QListensW(QWidget):
    ...
    def helptoimport(self,teacher,group,lesson):
        ...
        try:
            ...
            pass
    #проверяет евенты и обрабатывает их handker получается
    def CustomEventEnter(self):
        ...
        try:
            ...
            self.staticData.teacher = self.lineEditTeacher.text()
        return stackdata
    #ставит день недели по моему ну надо бы проверять
    def setdateweekday(self):
        ...
        if controlday >= startday:
            ...
            pass
      #TODO поправить даты  ???
       #откатывает данные в ячейках по поводу недели вперед
    def updateDateWeekdate(self):
     ...
     #откатывает данные в ячейках по поводу недели назад
    ...
    #обновляет то что не копипаститься вообще левая функция 
    #TODO сделать лучше
